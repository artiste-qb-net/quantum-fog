

class Controls:
    """
    This class stores a dictionary called self.bit2kind containing key-value
    pairs of the form (a control's qubit number: its kind). Kinds can be
    either a bool or a non-negative integer. Kind is True if control is P_1
    = n =|1><1|. Kind is False if control is P_0 = nbar = |0><0| control.
    Kind is a non-negative integer for multiplexor controls.

    See the docstring for the class SEO_reader to get an explanation of the
    meaning of the non-negative numbers used for the multiplexor kinds

    Attributes
    ----------
    num_bits : int
        number of qubits in full quantum circuit
    bit2kind : dict[int, bool|int]
        Dictionary matching control bit position with its kind.
        The domain of the map bit2kind is a subset of range(num_bits)
    bit_pos : list[int]
        This is the keys half if you unzip bit2kind.
    kinds : list[bool|int]
        this is the values half if you unzip bit2kind.

    """
    # combines my JAVA classes Control, TFControls, MultiControls

    def __init__(self, num_bits):
        """
        Constructor

        Parameters
        ----------
        num_bits : int

        Returns
        -------

        """
        self.num_bits = num_bits
        # num_controls = len(bit2kind) = len(bit_pos) = len(kinds)
        self.bit2kind = {}
        self.bit_pos = []
        self.kinds = []

    @staticmethod
    def new_from_old(old, extra_bits=0):
        """
        Create a copy of self but add extra_bit many uncontrolled bits at the
        end.

        Parameters
        ----------
        old : Controls
        extra_bits : int

        Returns
        -------
        Controls

        """
        new = Controls(old.num_bits + extra_bits)
        new.bit2kind = old.bit2kind
        new.bit_pos = old.bit_pos
        new.kinds = old.kinds
        return new

    def set_control(self, bit_pos, kind):
        """
        Add key-value pair (bit_pos: kind) to self.bit2kind dictionary

        Parameters
        ----------
        bit_pos : int
        kind : bool|int

        Returns
        -------
        None

        """
        assert -1 < bit_pos < self.num_bits, \
            "bit position is out of range"
        self.bit2kind[bit_pos] = kind

    def refresh_lists(self):
        """

        Replace the 2 lists bit_pos and kinds by unzipping (actually zip()
        does both zip and unzip) the dictionary bit2kind.

        Returns
        -------
        None

        """
        # li is list of tuples.
        # sort li items so that in decreasing order of first entry of tuples
        li = sorted(self.bit2kind.items(), key=lambda t: t[0], reverse=True)
        self.bit_pos, self.kinds = zip(*li)

    def is_control(self, bit_pos):
        """
        True if bit_pos in the keys of bit2kind. False otherwise.

        Parameters
        ----------
        bit_pos : int

        Returns
        -------
        bool

        """
        return bit_pos in self.bit_pos

    def get_workspace_bit_pot(self, target_bit_pos):
        """
        Find a bit that is neither the target bit nor one of the controls.
        Make it as big as possible.

        Parameters
        ----------
        target_bit_pos : int

        Returns
        -------
        int

        """
        num_controls = len(self.bit2kind)
        # must have room for num_controls + one target + one workspace bit
        assert self.num_bits >= num_controls + 2,\
            "must have more qubits to find a working space bit"

        li = [target_bit_pos] + self.bit_pos
        for k in reversed(range(self.num_bits)):
            if k not in li:
                break
        return k

    def get_num_int_controls(self):
        """
        Find number of controls that are not of boolean kind.

        Returns
        -------
        int

        """
        num = 0
        num_controls = len(self.kinds)
        for k in range(num_controls):
            if isinstance(self.kinds[k], int):
                num += 1
        return num

    def new_embedded_self(self, emb):
        """
        In bit2kind, replace each key by a new key emb.bit_map[key]. Also
        map the entries of bit_pos.

        Parameters
        ----------
        emb : CktEmbedder

        Returns
        -------
        Controls

        """
        if emb.is_identity():
            return self
        assert self.num_bits == emb.num_bits_bef,\
            "controls cannot be reset with CktEmbedder"
        new = Controls(emb.num_bits_aft)

        self.refresh_lists()
        num_controls = len(self.bit_pos)
        self.bit2kind = {emb.aft(self.bit_pos[c]): self.kinds[c] for
                    c in range(num_controls)}
        self.refresh_lists()
        return new

if __name__ == "__main__":
    print(5)


